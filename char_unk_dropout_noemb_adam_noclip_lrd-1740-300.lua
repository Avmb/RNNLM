...
require('nngraph')
require('base')
local ptb = require('char_unk_data')
require('LinearNoBias')
...
local optimizer_config = {learningRate=0.0002,
...
local function lstm(x, prev_c, prev_h, noise_i, noise_h)
  ...
  for i = 1, 4 do 
    ...
    local h2h_proj       = nn.LinearNoBias(params.rnn_size, params.rank_h)(dropped_h)
    local h2h_main       = nn.Linear(params.rank_h, params.rnn_size)(h2h_proj)
    --local h2h_main       = nn.Linear(params.rnn_size, params.rnn_size)(dropped_h)
    local h2h_d          = nn.CMul(params.rnn_size)(dropped_h)
    h2h[i]               = nn.CAddTable()({h2h_main, h2h_d})
    --h2h[i]               = h2h_main
  end
...
local function in_lstm(x, prev_c, prev_h, noise_i, noise_h)
  -- Reshape to (batch_size, n_gates, hid_size)
  -- Then slice the n_gates dimension, i.e dimension 2
  local reshaped_noise_i = nn.Reshape(4,params.rnn_size)(noise_i)
  local reshaped_noise_h = nn.Reshape(4,params.rnn_size)(noise_h)
  ...
  local reshaped_x       = nn.Reshape(4,params.rnn_size)(x)
  local sliced_x         = nn.SplitTable(2)(reshaped_x)
  -- Calculate all four gates 
  local i2h, h2h         = {}, {}
  for i = 1, 4 do 
    -- Use select table to fetch each gate
    local tmp            = nn.CAddTable()({nn.SelectTable(i)(sliced_x) , nn.MulConstant(0.0)(nn.SelectTable(i)(sliced_noise_i))}) -- ugly hack to avoid "unused node" errors
    i2h[i]               = tmp
    local dropped_h      = local_Dropout(prev_h, nn.SelectTable(i)(sliced_noise_h))
...
local function create_network()
  ...
  local i                = {[0] = LookupTable(params.vocab_size, params.rnn_size * 4)(x)}
  ...
  for layer_idx = 1, params.layers do
    ...
    local next_c, next_h
    if layer_idx == 1 then
      next_c, next_h = in_lstm(i[layer_idx - 1], prev_c, prev_h, n_i, n_h)
    else
      next_c, next_h = lstm(i[layer_idx - 1], prev_c, prev_h, n_i, n_h)
    end
...
local function setup()
  ...
  for j = 1, params.seq_length do
    ...
    model.noise_xe[j] = torch.expand(model.noise_x[j], params.batch_size, params.rnn_size * 4)
...
local function bp(state)
  ...
  local opfunc = function(paramx)
    return 1.0, paramdx -- Note: adam does not need the error value, only the gradient
  end
  optim.adam(opfunc, paramx, optimizer_config)
